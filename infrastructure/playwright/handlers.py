...
def wait_for_element(page, element_selector):
    try:
        page.wait_for_selector(element_selector, timeout=10000)  # Remplacez 'selector' par le sélecteur CSS spécifique
        print(f"L'élément avec le sélecteur '{element_selector}' a été trouvé.")
    except:
        print(f"L'élément avec le sélecteur '{element_selector}' n'a pas été trouvé après 10 secondes.")

def handle_banner(page):
    try:
        page.locator("text=Accepter").click(timeout=5000)  # Remplacez "Accepter" par le texte exact du bouton
    except:
